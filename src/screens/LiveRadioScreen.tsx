...
export const LiveRadioScreen = () => {
  ...
  const loadFavorites = async () => {
    try {
      const favs = await AsyncStorage.getItem('@radio_favorites');
      if (favs) {
  ...
  const toggleFavorite = async (channelId: string) => {
    ...
    try {
      await AsyncStorage.setItem('@radio_favorites', JSON.stringify(newFavs));
    } catch (e) {
      console.error('Failed to save favorite', e);
    }
  };

  const filteredChannels = radioChannels.filter((channel) =>
